...
function fop2_add_button($det) {
    ...
    $results = $db->consulta(
        "REPLACE INTO fop2buttons (type, device, label, exten, context, mailbox, queuechannel, customastdb, email, `group`, external, queuecontext, extenvoicemail,context_id,originatechannel,channel,accountcode) ".
        "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
        array($det['type'],$det['channel'],$name,$det['exten'],$context,$mailbox,$queuechannel,$customastdb,$email,$group,$external,$queuecontext,$extenvoicemail,$context_id,$originatechan,$extrachan,$accountcode)
    );
}
...
function fop2_del_user($id) {
    ...
    $results = $db->consulta("DELETE FROM fop2users WHERE id = '$id' $where");
}
...
function fop2_edit_setting($parameter,$value,$context) {
    $extension='SETTINGS';
    $db2 = new dbcon('sqlite:/usr/local/fop2/fop2settings.db');
    try {
...
function fop2_list_installed_plugins() {
    ...
    if(is_array($results)){
        foreach($results as $result){
            if(is_dir("plugins/".$result['rawname'])) {
                $allowed[] = $result;
            }
        }
    }
        ...
        while (false !== ($entry = readdir($handle))) {
            if ($entry != "." && $entry != "..") {
                if(is_dir($directory.$entry)) {
                     if(file_exists($directory.$entry.'/plugin.xml')) {
                         $data = file_get_contents($directory.$entry.'/plugin.xml');
                         //$xml = new xml2Array();
...
function plugin_read_xml_params($rawname) {
    $directory = dirname(__FILE__)."/plugins/";
    if(is_readable($directory.$rawname.'/plugin.xml')) {
        $data = file_get_contents($directory.$rawname.'/plugin.xml');
        //$xml = new xml2Array();
        //$xmlarray = $xml->parseAdvanced($data);
        $xml = simplexml_load_string($data);
        $xmlarray = simpleXMLToArray($xml);
...
function plugin_insert_missing_db() {
    ...
    foreach($plugin_local as $rawname=>$data) {
        if(!in_array($rawname,$plugsondb)) {
            ...
            if($data['global']=="yes") { $global=1; } else { $global=0; }
            $results = $db->consulta( "INSERT INTO fop2plugins (rawname, name, version, description, global) values ('%s','%s','%s','%s', '%s') ",
                                  array( $data['rawname'] ,$data['name'], $data['version'], $data['description'], $global ) );
        }
    }
}
...
function plugin_delete($itemid) {
    ...
    $rawname = $itemid;
    $directory = dirname(__FILE__)."/plugins/";
    $deldir = substr(escapeshellarg($directory.$rawname),1,-1);
    fop2_rrmdir($deldir);
...
function plugin_handleupload($uploaded_file) {
    ...
    exec("rm -rf ".$directory.$pluginname, $output, $exitcode);
    if ($exitcode != 0) {
        return array(sprintf(__('Could not remove old plugin %s to install new version'), $directory.$pluginname));
    }
    exec("mv ".$directory."/_cache/$pluginname ".$directory."/plugins/".$pluginname, $output, $exitcode);
    if ($exitcode != 0) {
        return array(sprintf(__('Could not move %s to %s'), $directory."/_cache/$pluginname", $directory));
    }
    exec("rm -rf ".$temppath);
    if ($exitcode != 0) {
...
function fop2_print_config_form($rawname) {
    ...
    $directory = dirname(__FILE__)."/plugins/";
    $inifile = $directory."$rawname/$rawname.ini";
    ...
    foreach ( $allpars['param'] as $key=>$param ) {
       ...
       $formname          = $param['name'];
       $formtype          = $param['type'];
       $formdefault       = $param['default'];
       $formmulti         = $param['multi'];
       $formencoded       = $param['encoded'];
       $fftype            = array();
       $fftype['string']  = 'text';
       ...
       if($formmulti=='yes') {
           ...
           $ret.="<label class='col-sm-2 control-label' for='$rawname-".$formname."-skeleton'>$finalname:</label>";
    ...
    $ret.="<button id='removesection-$rawname-skeleton' onClick='fop2_removeSection(this.id); return false;' class='btn btn-default'>".__('Remove Section')."</button><br/>\n";
    $ret.="</fieldset>\n\n";
...
function plugin_download($itemid, $mirror) {
    ...
    if($exitcode==0) {
        ...
        $infoxml   = substr(escapeshellarg($directory."$rawname/plugin.xml"),1,-1);
...
// This function will get a version string like 2.27.33.beta and returns a computable version like 022733
