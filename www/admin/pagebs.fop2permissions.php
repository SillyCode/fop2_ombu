...
<i style='vertical-align:super; top:-5px; color:#333;' class='ttip glyphicon glyphicon-info-sign' data-toggle='popover' data-trigger='hover' data-placement='bottom' data-content='<?php echo __("Restricted permissions lets you limit actions to a specific set of extensions/groups. They are only used for restricting actions. If you want a quick way to assign permissions, then you should use Templates as restricted permissions will limit user actions."); ?>'></i>
</div>
<div class='col-md-4 text-right'>
<form method='post' action='<?php echo SELF?>'><div class='btn-group'><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span></button><button class='btn btn-default'><?php echo __('Add Restricted Permission')?></button></div></form>
</div>
</div>
</div>
...
if (isset($perms)) {
    foreach ($perms as $d) {
        ...
        echo "<a style='color:#d11; cursor:pointer;' class='ttip' data-toggle='popover' data-trigger='hover' data-placement='left' data-content='".sprintf(__('Delete Restricted Permission %s'),$d['name'])."' onclick=\"setDelete('".urlencode($d['id'])."'); return false;\"><span class='glyphicon glyphicon-remove'></span></a>";
    }
}
...
<h2 class='fhead' style='height: 55px; z-index: 1000;'><?php echo ($itemid ? sprintf(__('Edit Restricted Permission %s'),$thisItem["name"]) : __('Add Restricted Permission')); ?>
...
<?php echo __('Restricted Permission Details')?>
    ...
    <label class="col-sm-3 control-label ttip" for="permname" data-toggle='popover' data-trigger='hover' data-placement='right' data-content='<?php echo __("The name for this permission.")?>'><?php echo __("Name")?></label>
    <div class="col-xs-8">
...
<span>2.  <?php echo __('Permissions')?></span>
  ...
  <div class="form-group">
    <label for="permissions" class="col-sm-3 control-label ttip" data-toggle='popover' data-trigger='hover' data-placement='right' data-content='<?php echo __('Choose the permission you want to grant to the user.')?>'><?php echo __('Permissions')?>
    </label>
        <div class="col-xs-8">
