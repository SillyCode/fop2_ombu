...
function mirtapbx_populate_contexts_from_tenants() {
    ...
    $results = $db->select('te_code,te_name','asterisk.te_tenants');
    if(is_array($results)) {
    ...
    $results = $db->select("id,context,name","fop2contexts","","exclude=0","");
    $return = array();
    if(is_array($results)) {
        foreach ($results as $result) {
...
function mirtapbx_check_extension_usage() {
    ...
    if(is_array($results)) {
        foreach ($results as $result) {
            ...
            $data['context']      = 'authenticated'; //$result['context'];
    ...
    // Park
    $results = $db->select("pk_name,pk_te_id,pk_start,fop2contexts.id as context_id","pk_parkinglots","LEFT JOIN fop2contexts on cast(te_code as char(50))=cast(fop2contexts.context as char(50))","","","","");
    if(is_array($results)) {
        foreach ($results as $result) {
