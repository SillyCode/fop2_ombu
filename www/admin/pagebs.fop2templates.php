...
<i style='vertical-align:super; top:-5px; color:#333;' class='ttip glyphicon glyphicon-info-sign'  data-toggle='popover' data-trigger='hover' data-placement='bottom' data-content='<?php echo __("A template is a set of permissions, groups and plugins that you can assign to users in one step inside the Users menu for easy and quick management."); ?>'></i>
</div>
<div class='col-md-4 text-right'>
<form method='post' action='<?php echo SELF?>'><div class='btn-group'><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span></button><button class='btn btn-default'><?php echo __('Add Template')?></button></div></form>
</div>
</div>
</div>
...
if(count($templates)>0) {
    foreach ($templates as $d) {
        ...
        echo "<div style='color:#d11; cursor:pointer;' class='ttip' data-toggle='popover' data-trigger='hover' data-placement='left' data-content='".sprintf(__('Delete Template %s'),$d['name'])."' onclick=\"setDelete('".urlencode($d['id'])."'); return false;\"><span class='glyphicon glyphicon-remove'></span></div>";
        echo "</td></tr>";
    }
}
...
<h2 class='fhead' style='height: 55px; z-index: 1000;'><?php echo ($itemid ? sprintf(__('Edit Template %s'),$thisItem["name"]) : __('Add Template')); ?>
...
<?php echo __('Template Details')?>
  ...
  <div class="form-group">
    <label for="templatename" class="col-sm-3 control-label ttip" data-toggle='popover' data-trigger='hover' data-placement='right' data-content='<?php echo __("The name for this template.")?>'><?php echo __("Name")?></label>
    <div class="col-xs-8">
