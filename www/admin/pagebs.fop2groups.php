...
<i style='vertical-align:super; top:-5px; color:#333;' class='ttip glyphicon glyphicon-info-sign' data-toggle='popover' data-trigger='hover' data-placement='bottom' data-content='<?php echo __("Groups can be used to restrict the buttons a user should see in his panel. You can create a group with only 5 extensions and assign it to a user in the Users tab, so he can only see those extensions."); ?>'></i>
</div>
<div class='col-md-4 text-right'>
<form method='post' action='<?php echo SELF?>'><div class='btn-group'><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span></button><button class='btn btn-default'><?php echo __('Add Group')?></button></div></form>
</div>
</div>
</div>
...
if (isset($groups)) {
    foreach ($groups as $d) {
        ...
        if(in_array($d,$predefined_groups)) {
            ...
            echo "<a style='color:#d11; cursor:pointer;' class='ttip' data-delay='{\"show\":\"750\", \"hide\":\"100\"}' data-toggle='popover' data-trigger='hover' data-placement='left' data-content='".sprintf(__('Delete Group %s'),$d['name'])."' onclick=\"setDelete('".urlencode($d['id'])."'); return false;\"><span class='glyphicon glyphicon-remove'></span></a>";
        }
        echo "</td></tr>";
    }
}
...
<h2 class='fhead' style='height: 55px; z-index: 1000;'><?php echo ($itemid<>'' ? sprintf(__('Edit Group %s'),$thisItem["name"]) : __('Add Group')); ?>
