...
foreach($panelcontexts as $panelcontext=>$contextname) {
    ...
    foreach($add_buttons as $chan=>$dat) {
        ...
        $results = $db->consulta(
                "REPLACE INTO fop2buttons (type, device, label, exten, context, mailbox, queuechannel, customastdb, email, `group`, external, queuecontext, extenvoicemail, originatechannel, context_id, channel) ".
                "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s', '%s')",
                array($det['type'],$det['channel'],$det['name'],$det['exten'],$det['context'],$mailbox,$queuechannel,$customastdb,$email,$group,$external,$queuecontext,$extenvoicemail,$originatechannel,$context_id,$extrachan)
                );
    }
