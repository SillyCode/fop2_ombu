...
<i style='vertical-align:super; top:-5px; color:#333;' class='ttip glyphicon glyphicon-info-sign'  data-toggle='popover' data-trigger='hover' data-placement='bottom' data-content='<?php echo __('From here you can install, uninstall, upload and configure FOP2 plugins. You can get plugins from FOP2 repositories, or upload and manage local plugins.'); ?>'></i>
</div>
</div>
</div>
...
<?php
	...
	foreach ($plugin_sorted as $idx=>$plugname) {
        ...
        if($superversion == $superversionlocal) { 
            ...
            $status.= "<button class='btn btn-danger col-md-12' onclick='setUninstall(\"$rawname\"); return false;'><i class='glyphicon glyphicon-floppy-remove pull-left'></i>&nbsp;".__('Uninstall')."</button>\n";
            $status.= "</form>\n";
            ...
            if($price==0 || ($price>0 && $paid==1)) {
                ...
                if($fop2version>0 && $fop2version>=$fop2minversion) {
                    ...
                    $status.= "<button class='btn btn-success col-md-12' onclick='getPlugin(\"$rawname-$plugversion\"); return false;'><i class='glyphicon glyphicon-floppy-save pull-left'></i>&nbsp;".__('Install')."</button>\n";
                    $status.= "</form>";
                } else {
                ...
                if($fop2version>0 && $fop2version>=$fop2minversion) {
                    ...
                    $status = "<a class='btn btn-default col-md-12' href='$geturl' onclick='window.open(this.href); return false;'><i class='glyphicon glyphicon-shopping-cart pull-left'></i>&nbsp;".__('Buy')."</a>";
                } else {
                    $status = "<span class='label label-danger'>".__('Not compatible with this version')."</span>"; 
                }
            }
        ...
        echo "<div class='col-md-3'><strong>$name</strong></div>\n";
        ...
        if($superversionlocal>0) {    // Esta intalado
            ...
            if(count($configparams)>0) {  // Tiene parametros de configuracion
                ...
                echo "<div class='row' style='background-colo:#B4DBB3; margin:0; padding:10px;'>\n";
                ...
                echo "<i class='glyphicon glyphicon-cog pull-left'>";
...
<input type="file" class='control-form' name="uploadplug" /> &nbsp;&nbsp; <button class='btn btn-default' type="submit"><i class='glyphicon glyphicon-upload pull-left'></i>&nbsp;<?php echo __('Upload');?></button>
</form>
  </div>
</div>
</div>
...
$(document).ready(function() {
    ...
    $('.file-input-wrapper').find('span').html('<i class="glyphicon glyphicon-list-alt pull-left"></i>&nbsp;<?php echo __('Browse');?>');
